this.ckan.module('recline_view',function(jQuery){var NoRecordsView=Backbone.View.extend({render:function(){this.$el.html('<div class="recline-norecords">No matching records</div>');}});return{options:{site_url:"",controlsClassName:"controls",dataproxyUrl:"//jsonpdataproxy.appspot.com"},initialize:function(){jQuery.proxyAll(this,/_on/);this.options.resource=JSON.parse(this.options.resource);this.options.resourceView=JSON.parse(this.options.resourceView);this.el.ready(this._onReady);L.Icon.Default.imagePath=this.options.site_url+'vendor/leaflet/0.7.7/images';},_onReady:function(){var resourceData=this.options.resource,resourceView=this.options.resourceView;setTimeout(this.waitForViews.bind(this,0,resourceData,resourceView),0);},waitForViews:function(attempts,resourceData,resourceView){if((window.parent.ckan&&window.parent.ckan.views&&window.parent.ckan.views.filters)||attempts>=100){this.loadView(resourceData,resourceView);}else{setTimeout(this.waitForViews.bind(this,attempts+1,resourceData,resourceView),50);}},loadView:function(resourceData,reclineView){var self=this;function showError(msg){msg=msg||self._('error loading view');window.parent.ckan.pubsub.publish('data-viewer-error',msg);}
...
var defaultFilters=reclineView.filters||{},filters=jQuery.extend({},defaultFilters,urlFilters);jQuery.each(filters,function(field,values){query.addFilter({type:'term',field:field,term:values});});if(window.parent.ckan.views.filters._searchParams.q){query.set({q:window.parent.ckan.views.filters._searchParams.q});}
dataset.queryState.set(query.toJSON(),{silent:true});errorMsg=this._('Could not load view')+': ';if(resourceData.backend=='ckan'){errorMsg+=this._('DataStore returned an error');}else if(resourceData.backend=='dataproxy'){errorMsg+=this._('DataProxy returned an error');}
dataset.fetch().done(function(dataset){self.initializeView(dataset,reclineView);}).fail(function(error){if(error.message)errorMsg+=' ('+error.message+')';showError(errorMsg);});},initializeView:function(dataset,reclineView){var view,state,controls=[];if(typeof(dataset.recordCount)=='undefined'||dataset.recordCount==0){view=new NoRecordsView({'model':dataset});}else if(reclineView.view_type==="recline_graph_view"){state={"graphType":reclineView.graph_type,"group":reclineView.group,"series":[reclineView.series]};view=new recline.View.Graph({model:dataset,state:state});}else if(reclineView.view_type==="recline_map_view"){state={geomField:null,latField:null,lonField:null,autoZoom:Boolean(reclineView.auto_zoom),cluster:Boolean(reclineView.cluster_markers)};if(reclineView.map_field_type==="geojson"){state.geomField=reclineView.geojson_field;}else{state.latField=reclineView.latitude_field;state.lonField=reclineView.longitude_field;}
view=new recline.View.Map($.extend(this._reclineMapViewOptions(dataset,this.options.map_config),{state:state}));}else if(reclineView.view_type==="recline_view"){view=this._newDataExplorer(dataset,this.options.map_config);}else{if(reclineView.state===undefined){reclineView.state={}}else{jQuery.each(['defaultFormatter','formatterFactory','editorFactory'],function(i,value){if(reclineView.state['gridOptions'][value]!==undefined){reclineView.state['gridOptions'][value]=window[reclineView.state['gridOptions'][value]];}});}
view=new recline.View.SlickGrid({model:dataset,state:reclineView.state});controls=[new recline.View.Pager({model:view.model}),new recline.View.RecordCount({model:dataset}),new recline.View.QueryEditor({model:view.model.queryState})];}
